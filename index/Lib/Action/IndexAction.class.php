...
class IndexAction extends BasicAction {
    public function index(){
    	...
    	$news_id="104101";
    	$db=M("info");
    	$hot=$db->where("class_id='$news_id' and pic<>'' and state>0")->order("sortnum desc")->limit(1)->select();
    	$hot=$hot[0];
    	...
    	$comNews=$db->where("class_id='$news_id' and state>0 and id<>{$hot['id']}")->field("id,title")->order("sortnum desc")->limit(2)->select();
    	...
    	foreach($psclass as $key=>$ps){
    		$s_id=$ps['id'];
    		$s_info=$db->where("class_id =$s_id and state>0 and pic<>''")->order("state desc,sortnum desc")->limit(1)->select();
            $info=$s_info[0];
